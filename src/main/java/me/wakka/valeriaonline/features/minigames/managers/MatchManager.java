...
public class MatchManager {
	...
	public static Match getActiveMatchFromLocation(Mechanic mechanic, Location location) {
		for (ProtectedRegion region : new WorldGuardUtils(location).getRegionsAt(location)) {
			Arena arena = me.pugabyte.bncore.features.minigames.managers.ArenaManager.getFromRegion(region.getId());
			if (arena == null) continue;

			Match match = me.pugabyte.bncore.features.minigames.managers.MatchManager.get(arena);
