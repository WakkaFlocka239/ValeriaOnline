...
public class VOScoreboard {
	private static final ScoreboardManager manager = ValeriaOnline.getInstance().getServer().getScoreboardManager();
	...
	public VOScoreboard(String title) {
		this(title, title, Collections.emptyList());
	}

	public VOScoreboard(String title, Player player) {
		this(title, title, Collections.singletonList(player));
	}

	public VOScoreboard(String title, Collection<? extends Player> players) {
		this(title, title, players);
	}

	public VOScoreboard(String id, String title) {
		this(id, title, Collections.emptyList());
	}

	public VOScoreboard(String id, String title, Player player) {
		this(id, title, Collections.singletonList(player));
	}

	public VOScoreboard(String id, String title, Collection<? extends Player> players) {
		try {
			scoreboard.removeObjective(left(id, 16));
