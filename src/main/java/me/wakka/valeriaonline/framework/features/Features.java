...
import me.wakka.valeriaonline.framework.annotations.Disabled;
...
public class Features {
	...
	public static <T extends Feature> T get(Class<? extends Feature> clazz) {
		return (T) Optional.of(features.get(clazz)).orElseThrow(() -> new CustomException("Feature " + prettyName(clazz) + " not found"));
	}
	...
	public void register(Feature feature) {
		...
		new Time.Timer("  Register feature " + feature.getName(), () -> {
			try {
				feature.startup();
	...
	public void unregister(Feature feature) {
		new Time.Timer("  Unregister feature " + feature.getName(), () -> {
			try {
				feature.shutdown();
			} catch (Exception ex) {
